use engram::{ItemMemory, encode_image, encode_image2, predict};
...
fn main() -> Result<(), MnistError> {
    ...
    let train_hvs: Vec<BinaryHDV<M>> = data
        .train_images
        .iter()
        .map(|im| encode_image2(im.as_u8_array(), &imem))
        .collect();

    println!("Encoding test images...");
    //let test_hvs: Vec<BinaryHDV<N>> = data
    let test_hvs: Vec<BinaryHDV<M>> = data
        .test_images
        .iter()
        .map(|im| encode_image2(im.as_u8_array(), &imem))
        .collect();
    ...
    let n_epochs = 10;
    let mut accumulators: [BinaryAccumulator<M>; 10] =
        core::array::from_fn(|_| BinaryAccumulator::<M>::new());
    ...
    if n_epochs > 1 {
        ...
        for epoch in 2..=n_epochs {
            ...
            let models: [BinaryHDV<M>; 10] = core::array::from_fn(|i| accumulators[i].finalize());
            let mut errors = 0;
